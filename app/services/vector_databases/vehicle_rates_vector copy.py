...
class VehicleVectorDB:
    ...
    def _index_data(self, csv_path):
        ...
        for idx, row in df.iterrows():
            ...
            # MAKE,MODEL,SERIES,OPTIONPACKAGE,BODYSTYLE,Wheelbase,ENGINE,GRG,DRG,VSD,LRG,YEAR 
            # Create semantic string
            semantic_text = (
                f"Year: {clean(row.get('YEAR'))}"
                f"Make: {row['MAKE']} " 
                f"Model: {clean(row['MODEL'])}  "
                f"Description {clean(row.get('SERIES'))} " 
                f" {clean(row.get('OPTIONPACKAGE'))} " 
                f" {clean(row['BODYSTYLE'])} "
                f" {clean(row.get('ENGINE'))} " 
                f" {clean(row.get('WHEELBASE'))} " 
                f"Ratings - GRG: {row['grg']}, DRG: {row['drg']} VSD: {row['vsd']} LRG: {row['lrg']}"  
            )
    ...
    def search_with_boosting(self, query_text, boost_targets, weights, top_k=5):
        ...
        if results['ids'] and len(results['ids']) > 0:
            ...
            target_description_tokens = self._tokenize_model(boost_targets.get('description', ''))
            ...
            for i in range(len(results['ids'][0])):
                ...
                candidate_description  = meta.get('description', '')
